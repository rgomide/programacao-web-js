...
<div style="margin-top: 1.2rem; display: flex; align-items: center; gap: 1.5rem; justify-content: flex-end; padding-right: 8%;">
  <img src="../04-manipulacao-de-dados/assets/nodejs-logo.svg" alt="Node.js" height="72" />
  <img src="../04-manipulacao-de-dados/assets/javascript-logo.png" alt="JavaScript" width="72" height="72" />
  <img src="../04-manipulacao-de-dados/assets/postgresql-elephant.png" alt="PostgreSQL" width="72" height="72" />
</div>
...
Analogia: é como um **ingresso de show com holograma**. O organizador (servidor) emite; a portaria (middleware) confere se o holograma bate com o que só o organizador sabe fabricar.
...
<div class="stack-row">
  ...
  <figure>
    <img src="../assets/img/05-jwt/jwt-anatomy.svg" alt="JWT" />
    <figcaption><small>jsonwebtoken</small></figcaption>
  </figure>
</div>

Bibliotecas principais: **`jsonwebtoken`** (criar/validar tokens) e **`bcryptjs`** (hash de senhas).

---

## Anatomia de um token

![w:900](../assets/img/05-jwt/jwt-anatomy.svg)
...
> O **payload não é criptografado** — apenas **codificado**. Qualquer um pode **ler**; a assinatura impede **alterar** sem ser detectado.
...
![w:900](../assets/img/05-jwt/auth-flow.svg)

1. **Registro** — senha hasheada antes de ir ao banco
2. **Login** — credenciais válidas → servidor devolve `{ token }`
3. **Rotas protegidas** — cliente envia token; middleware valida antes do handler
...
Analogia: **checkpoints** na fila antes do guichê. Algo errado? Manda embora (`401`). OK? `next()`.
...
![w:900](../assets/img/05-jwt/middleware-flow.svg)
...
Sem o cabeçalho → `{ "erro": "Token não informado" }`.

---

## Boas práticas de segurança

1. **`JWT_SECRET` forte** — `npm run generate:jwt-secret`; nunca no Git
...
- Exercícios no [README.md](./README.md): proteger `POST /alunos`, autorização (`403`), middleware opcional
- Distinuir token **expirado** vs **inválido** (código `TOKEN_EXPIRADO`)
...
- Código e exercícios: [README.md](./README.md)
